...
const Dropdown: FC<DropdownProps> = ({ menuItems, showColHeader }) => {
  return (
    <div>
      ...
      <div
        className={`bg-white relative bg-white rounded-[10px] shadow-[1px_1px_25px_0_rgba(0,0,0,0.08)] py-[47px] px-[39px] gap-x-[30px] grid grid-flow-col`}
      >
        {menuItems &&
                ...
                {items.items.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Typography
                        variant="body2"
                        className={`font-bold capitalize primary text-gray ${
                          item.icon && "ml-[15px]"
                        }`}
                      >
                        {item?.title}
